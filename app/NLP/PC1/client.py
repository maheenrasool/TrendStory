...
PC0_IP =  "192.168.18.107" #"192.168.18.107"
SERVER_PORT = 50052 
...
def get_trend_story(tones, themes, styles, language):
    if not tones or not themes or not styles:
        return "⚠️ Please select at least one option from each category."
    ...
    try:
        ...
        image = Image.open(io.BytesIO(response.image_data)) if response.image_data else None
        return response.response, image  # .response contains the story and image is the trend graph
...
with gr.Blocks(theme=purple_theme, title="gRPC TrendStory Generator") as demo:
    ...
    <style>
        button:hover {
            background-color: #E6CCE6 !important;
            color: #000 !important;
    ...
    generated_image = gr.Image(label="Generated Trend Graph")
    # # 📈 Trend graph display
    # gr.Image(value=r"Trend Graph.gif", label="📈 Trend Graph", show_label=True, interactive=False)
    generate_btn.click(
        fn=get_trend_story,
        inputs=[tones, themes, styles, language],
        outputs=[story_output, generated_image]
    )
    ...
    scroll_html = """
    <div style='height: 500px; overflow-y: auto; display: flex; flex-direction: column; gap: 16px; padding: 10px;'>
    """
...
demo.launch(share=False) # share = False when you don't want to use the internet
